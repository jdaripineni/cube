...
/* configure's WITH_CRC32C_HW check (mkl_compile_check) only verifies that the
 * x86 SSE4.2 inline assembly below (cpuid/crc32b/crc32q) *compiles* in
 * isolation; on some non-x86 hosts (observed on arm64 macOS/clang) that
 * isolated check passes even though it cannot possibly run, and the full
 * build then fails elsewhere in this file with an invalid asm operand.
 * Force the hardware path off on any architecture it wasn't written for,
 * regardless of what configure detected. See the FIXME above re: ARM. */
#if WITH_CRC32C_HW && \
    !(defined(__x86_64__) || defined(__i386__) || defined(_M_X64) || \
      defined(_M_IX86))
#undef WITH_CRC32C_HW
